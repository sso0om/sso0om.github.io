...
정렬이 락 순서를 지켜준다는 가정도, 락이 SKU에만 걸린다는 가정도 실측 앞에서 어긋났다. 문제 A(락의 획득 순서로 인한 데드락 위험)는 아직 해결된 게 아니고, 문제 B(FOR UPDATE + JOIN 구조 자체의 문제)는 확인됐다. 다음 글에서는 이 상태를 동시성 통합 테스트로 재현해, 정황이 아니라 실제 동시 실행에서도 같은 결론이 나오는지 본다.
